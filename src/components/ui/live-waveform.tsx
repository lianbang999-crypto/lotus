...
export type LiveWaveformProps = HTMLAttributes<HTMLDivElement> & {
  ...
  /**
   * 复用调用方已经开好的麦克风流。传了就不再自己 getUserMedia，
   * 也不会去 stop 它的轨道——流的所有权留在开它的那一方（这里是录音机）。
   * 不传则维持原行为：自己开麦、自己收。
   */
...
export const LiveWaveform = ({
  ...
  useEffect(() => {
    ...
    const setupMicrophone = async () => {
      try {
        // 借来的流直接用；只有没人给流时才自己申请麦克风（会弹第二次授权）。
        const stream =
          externalStream ??
        ...
        ownsStreamRef.current = !externalStream
        streamRef.current = stream
        if (ownsStreamRef.current) onStreamReady?.(stream)
